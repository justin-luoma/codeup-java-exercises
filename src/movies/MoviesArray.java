...
public class MoviesArray {
    ...
    static String[] moviesToString(String category) {
        Movie[] movies = findAll();
        movies = Arrays.stream(movies)
                .filter(m -> (m.movieCategory().equals(category)))
                .toArray(Movie[]::new);
        String[] movieStrings = new String[movies.length];
        for (int i = 0; i < movies.length; i++) {
            String mCategory = movies[i].movieCategory();
            if (mCategory.equals(category)) {
                movieStrings[i] = movies[i].movieName() + " -- " + mCategory;
            }
        }
        return movieStrings;
    }

    private static Movie[] findAll() {
        return movies;
    }
